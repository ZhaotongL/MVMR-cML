...
// initialize
  ...
  for(unsigned int random_ind = 0; random_ind < random_start; ++random_ind)
  {
    if(random_ind == 0)
    {
      ...
      initial_theta = runif(k,  -0.5,  0.5); // TO-DO: option to change initial value for theta
      arma::mat b_rand;
                      ...
                      const double thres = 1e-4)
{
  ...
  Rcpp::List cML_res = MVmr_cML(b_exp, b_out, se_bx, Sig_inv_l, n, K_vec, random_start, maxit, thres);
  arma::mat b_exp_new(size(b_exp)), b_out_new(size(b_out));
  arma::vec theta_b, Conv_v(num_pert);
  for(unsigned int pt_ind = 0; pt_ind < num_pert; ++pt_ind)
    ...
    for(unsigned int i = 0; i < p; ++i)
      ...
      Rcpp::List cML_res_b = MVmr_cML(b_exp_new, b_out_new, se_bx, Sig_inv_l, n, K_vec, random_start, maxit, thres);
